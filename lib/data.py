...
def get_mmap_dataset(path='./dataset/ogbn-products-new', split_idx_path=None):
...
class NewDataset():
    def __init__(self, path='./dataset/ogbn-products-new', split_idx_path=None, score_path=None):
        ...
        split_idx = torch.load(split_idx_path)
        if isinstance(split_idx['train'], np.ndarray):
            self.train_idx = torch.from_numpy(split_idx['train'])
        else:
            self.train_idx = split_idx['train']
        if isinstance(split_idx['valid'], np.ndarray):
            self.val_idx = torch.from_numpy(split_idx['valid'])
        else:
            self.val_idx = split_idx['valid']

        if 'mag240m' not in path:
