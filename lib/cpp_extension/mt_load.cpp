...
torch::Tensor load_float32(std::string file, int64_t size1, int64_t size2){

    // open file
    int fd = open(file.c_str(), O_RDONLY | O_DIRECT);

    int64_t result_buffer_size = size1*size2*sizeof(float)+ALIGNMENT;
    ...
    auto options = torch::TensorOptions()
        ...
        .device(torch::kCPU)
        .requires_grad(false);
    auto result = torch::from_blob(result_buffer, {size1, size2}, options);
